...
# Usage: 
# count_cons_seqs.1.r --DR_filename=~/project/TR02/TR02.26/run1/aa_DetectRepeats/K24316.DetectRepeats.rds --MSA_filename=~/project/TR02/TR02.26/run1/aa_MSA/K24316.MSA.rds --score=15 --out_rds=~/project/TR02/TR02.26/run1/aa_count_cons_seqs/K24316.tsv
...
option_list = list(
	...
	make_option(c("-o", "--out_rds"), type="character", default="NA", help="output tsv (Required)", metavar="filename")
); 

# get command line options, if help option encountered print help and exit,
# otherwise if options not found on command line then set defaults,
opt <- parse_args(OptionParser(usage = "%prog [options] [--DetectRepeat_dir=FILENAME] [--MSA_dir=FILENAME] [--score=FLOAT] [--out_rds=FILENAME]", option_list=option_list)) 
        ...
        if (!is.na(PID))
        {
            if (PID >= 0.5 && PID <= 0.6)
            {
                ...
                if ( !all(is.na(pair_df[pair_df[,1] == 1, 2])) && !all(is.na(pair_df[pair_df[,2] == 1, 1]))) # exclude if repeat positions of one seq are all NA in another
                {
                    ...
                    {
                        out_mt[pairs[,pair_h],'cons_repeat'] <- 1 # seqs have consistent repeats
                    } 
                }
            }
        }
...
saveRDS(out_mt[out_mt[,'in_pairs']==1, c(1,3)], file=opt$out_rds, compress = TRUE)
